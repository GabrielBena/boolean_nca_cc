...
def plot_training_curves(metrics, title, output_dir):
    ...
    # Plot soft loss
    axs[0, 0].plot(metrics["losses"], label=title)
    axs[0, 0].set_title("Soft Loss")
    ...
    # Plot hard loss
    axs[0, 1].plot(metrics["hard_losses"], label=title)
    axs[0, 1].set_title("Hard Loss")
    ...
    # Plot soft accuracy
    axs[1, 0].plot(metrics["accuracies"], label=title)
    axs[1, 0].set_title("Soft Accuracy")
    ...
    # Plot hard accuracy
    axs[1, 1].plot(metrics["hard_accuracies"], label=title)
    axs[1, 1].set_title("Hard Accuracy")
    axs[1, 1].set_xlabel("Training Steps")
    axs[1, 1].set_ylabel("Accuracy")
    axs[1, 1].legend()
    axs[1, 1].grid(True)

    plt.suptitle(f"{title} Training Curves")

    # Save the figure
    plot_path = os.path.join(output_dir, f"{title.lower().replace(' ', '_')}_curves.png")

    if wandb.run is not None:
        wandb.log({f"{title} Training Curves": wandb.Image(fig)})
