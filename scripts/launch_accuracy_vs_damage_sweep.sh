#!/usr/bin/env bash
# Launch accuracy_vs_damage_plot.py for the 5 finished runs in sweep f1pktfh5.
...
RUN_IDS=(ap1b5iyb 0kbp45do 43mjb3pf brt24qbj ig4cnjty)
PAT_SEEDS=(1014    1123     1042     1456     1789)
...
OUT_ROOT=reports/figures/rev_damage_scale
SWEEP_TS=$(date +%Y%m%d_%H%M%S)
LOG_DIR=logs/accuracy_vs_damage_sweep_${SWEEP_TS}
mkdir -p "${LOG_DIR}"
MANIFEST="${LOG_DIR}/manifest.txt"
echo "Logs: ${LOG_DIR}"

# Pre-compute output dirs (must match accuracy_vs_damage_plot.py's default naming
# template for damage_behavior=reversible).
KO_PART="${KO_SIZES//,/_}"
OUT_DIRS=()
for run_id in "${RUN_IDS[@]}"; do
    OUT_DIRS+=("${OUT_ROOT}/accuracy_scaled${KO_PART}_p${PATTERNS_PER_SIZE}_reversible_run${run_id}_${SWEEP_TS}")
done
...
run_one() {
    ...
    python experiments/accuracy_vs_damage_plot.py \
        --run-id "${run_id}" \
...
echo "  python experiments/accuracy_vs_damage_REplot_multi.py \\"
echo "    --manifest ${MANIFEST} \\"
