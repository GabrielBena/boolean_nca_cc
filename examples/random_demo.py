...
from imgui_bundle import implot, imgui_knobs, imgui, immapp, imgui_ctx, immvision, hello_imgui
...
def loss_f(logits, wires, gate_mask, x, y0):
    ...
    return loss, dict(act=act, err_mask=err_mask,
        hard_loss=hard_loss, hard_act=hard_act)
...
class Demo:
    def __init__(self):
        ...
        self.inputs_img = zoom(self.input_x.T, 4)
        self.active_case_i = 123
        
        self.tasks = dict(copy=x, gray=x^(x>>1),
            add4=(x&0xf)+(x>>4), mul4=(x&0xf)*(x>>4),
    ...
    def mask_unused_gates(self):
        gate_masks, self.wire_masks = calc_gate_use_masks(input_n, self.wires, self.logits)
    ...
    def update_circuit(self):
        ...
        if self.is_training:
            upd, self.opt_state = self.get_opt().update(grad, self.opt_state, self.logits)
        ...
        for li, (gate_n, group_size) in enumerate(layer_sizes):
            ...
            for i, x in enumerate(gate_x):
                ...
                if is_point_in_box(p0, p1, io.mouse_pos):
                    ...
                    if li > 0:
                        hover_gate = x, y, self.logits[li-1][i//group_size,i%group_size]
            ...
            for x in group_x[:,0]:
                dl.add_rect(
                     (x-group_w/2, y-d/2), (x+group_w/2, y+d/2), 0x80ffffff, 4)

            if li > 0:
                wires = self.wires[li-1].T
                masks = self.wire_masks[li-1].T
                src_x = prev_gate_x[wires]
                dst_x = group_x + (np.arange(arity)+0.5)/arity * group_w - group_w/2
                my = (prev_y+y)/2
                for x0, x1, si, m in zip(src_x.ravel(), dst_x.ravel(), wires.ravel(), masks.ravel()):
                    if not m:
                        continue
                    a = int(prev_act[si]*0x60)
                    col = 0xff404040+(a<<8)
                    dl.add_bezier_cubic((x0, prev_y+d/2), (x0,my), (x1,my), (x1, y-d/2), 
                        col, 1.0)
    ...
    def draw_lut(self, name, img):
        ...
        mx, _ = immvision.image_display_resizable(name, img, (view_w, 0),
            resizable=False, refresh_image=True)
    ...
    def gui(self):
        ...
        implot.begin_plot('Train logs', (-1, 200))
        ...
        implot.drag_line_x(1, self.trainstep_i%len(self.loss_log), (0.8,0,0,0.5))
        implot.end_plot()
        ...
        if imgui.button('shuffle wires'):
            ...
            self.shuffle_wires()
            self.trainstep_i = 0
        local_noise_changed, self.local_noise = imgui.slider_float('local noise', self.local_noise, 0.0, 20.0)
        ...
        task_changed, self.task_idx = imgui.combo("task",  self.task_idx, self.task_names)
        if task_changed:
            self.update_task()
            self.trainstep_i = 0
        task_name = self.task_names[self.task_idx]
        if task_name == 'text':
            ...
            if text_changed:
                self.update_task()
        if task_name == 'noise':
            noise_changed, self.noise_p = imgui.slider_float('p', self.noise_p, 0.0, 1.0)
            if noise_changed:
                self.update_task()            
...
if __name__ == "__main__":
    ...
    immapp.run(demo.gui, window_title="Random boolean circuit",
        #window_size=(800, 600), 
        window_size_auto=True,
        window_restore_previous_geometry=True,
        fps_idle=10,
        with_implot=True)  # type: ignore
