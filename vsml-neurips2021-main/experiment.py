...
class Experiment:
    ...
    def evaluate(self, log_to_wandb=True):
        ...
        for name, dataset in zip(dataset_names, self._eval_datasets):
            ...
            if self.rank == 0:
                ...
                dfs.append(df)
                log[f'eval_loss_{name}'] = loss.item()
                log[f'eval_accuracy_{name}'] = accuracy.item()
                log[f'eval_final_loss_{name}'] = final_loss.item()
                log[f'eval_final_accuracy_{name}'] = final_accuracy.item()
    ...
    def step(self, global_step, rng):
        ...
        scalars = {k: v.item() for k, v in scalars.items()}
        return scalars
    ...
    def _es_eval(self, params, images, labels, rng, noise_std):
        # TODO use antithetic sampling from jax.jit examples
        # TODO vmap over multiple noise samples
        rng_key_1, rng_key_2 = jax.random.split(rng)
        treedef = jax.tree_util.tree_structure(params) # Corrected
        # Generate noise with the same treestructure as params
        noise_struct = jax.tree_util.tree_map(lambda x: jax.random.normal(rng_key_1, x.shape), params)
        scaled_noise = jax.tree_util.tree_map(lambda x: x * noise_std, noise_struct) # Corrected

        # Antithetic sampling
        params_pos = jax.tree_util.tree_map(lambda mean, val: mean + val, params, scaled_noise)
        params_neg = jax.tree_util.tree_map(lambda mean, val: mean - val, params, scaled_noise)

        # TODO Multiple evaluations via vmap
        loss_pos = self._outer_loss(params_pos, images, labels, rng_key_2)
        loss_neg = self._outer_loss(params_neg, images, labels, rng_key_2)

        es_factor = (loss_pos - loss_neg) / (2 * noise_std ** 2)
        grads = jax.tree_util.tree_map(lambda x: x * es_factor, scaled_noise)
        return grads, loss_pos
